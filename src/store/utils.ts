...
/**
 * Selector optimizado para extraer slices específicas del store
 * Previene re-renders innecesarios
 *
 * @example
 * const theme = useStore(selectTheme);
 */
export function selectSlice<T, K extends keyof T>(slice: K) {
  return (state: T): T[K] => state[slice];
}

/**
 * Crea un selector memoizado para valores computados
 *
 * @example
 * const selectFilteredTasks = createSelector(
 *   (state: TaskStore) => state.tasks,
 *   (state: TaskStore) => state.filters,
 *   (tasks, filters) => tasks.filter(t => matchesFilter(t, filters))
 * );
 */
export function createSelector<TState, TResult, TDeps extends unknown[]>(
  ...args: [
    ...{ [K in keyof TDeps]: (state: TState) => TDeps[K] },
    (deps: TDeps) => TResult,
  ]
): (state: TState) => TResult {
  const selectors = args.slice(0, -1) as {
    [K in keyof TDeps]: (state: TState) => TDeps[K];
  };
  const compute = args[args.length - 1] as (deps: TDeps) => TResult;

  return (state: TState): TResult => {
    const deps = selectors.map((s) => s(state)) as TDeps;
    return compute(deps);
  };
}
