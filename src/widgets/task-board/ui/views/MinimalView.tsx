    ...
    <div className="space-y-0">
      <AnimatePresence mode="popLayout">
        {sortedTasks.map((task, index) => (
          ...
          >
            <div
              className={`border-b border-border/10 py-3 px-2 hover:bg-background/30 transition-all rounded-lg ${
                task.isCurrent
                  ? `${classes.gradientBgSubtle} border-l-2 ${classes.borderHover}`
                  : ""
              }`}
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    {task.isCurrent && !task.isCompleted && (
                      <span
                        className={`text-xs px-1.5 py-0.5 rounded ${classes.gradientBgSubtle} ${classes.textPrimary} font-medium`}
                      >
                        Active
                      </span>
                    )}
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  {!task.isCurrent && !task.isCompleted && (
                    ...
                    >
                      <Target
                        className={`h-3.5 w-3.5 ${classes.textPrimary}`}
                      />
                    </motion.button>
                  )}
                  <motion.button
