        ...
        {sortedTasks.map((task, index) => {
          ...
          return (
            <motion.div
              ...
              exit={{ opacity: 0, scale: 0.8, rotate: rotation + 3 }}
              transition={{ duration: 0.4, delay: index * 0.05 }}
              whileHover={{ scale: 1.05, rotate: 0, zIndex: 10 }}
              className="group"
            >
              <div
                className={`rounded-lg border-2 p-4 h-full flex flex-col shadow-lg ${getPostItColor(index)} ${
                  task.isCurrent
                ...
                <div className="flex items-start gap-2 mb-2">
                  ...
                  <div className="flex-1 min-w-0">
                    {task.isCurrent && !task.isCompleted && (
                      <span
                        className={`text-xs px-1.5 py-0.5 rounded ${classes.gradientBgSubtle} ${classes.textPrimary} font-medium mb-1 inline-block`}
                      >
                        Active
                      </span>
                    )}
