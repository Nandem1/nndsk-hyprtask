...
interface TerminalOutViewProps {
  tasks: Task[];
...
export function TerminalOutView({
  tasks,
  onToggle,
  onDelete,
  onSetCurrent,
    ...
    <div className="space-y-1 font-mono text-sm">
      <AnimatePresence mode="popLayout">
        {sortedTasks.map((task, index) => (
          ...
          >
            <div
              className={`glass rounded border border-border/20 p-3 hover:border-border/40 transition-all ${
                task.isCurrent
            ...
            >
              <div className="flex items-center gap-2">
                <span className={`${classes.textPrimary}`}>
                  [{task.isCompleted ? "✓" : task.isCurrent ? ">" : " "}]
                </span>
                <motion.button
      ...
      {!showForm && canAddTask && (
        ...
        >
          ...
          >
            <Plus className="h-4 w-4 mr-2" />$ add_task
          </Button>
        </motion.div>
      )}
      ...
      {tasks.length === 0 && !showForm && (
        <div className="glass rounded border border-border/20 p-8 text-center font-mono">
          <p className="text-foreground text-base font-medium mb-1">
            {totalCount === 0 ? "$ no tasks" : "$ no tasks match filters"}
          </p>
